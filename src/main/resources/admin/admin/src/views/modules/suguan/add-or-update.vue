	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="宿管职级" prop="suguanzhiji">
					<el-select :disabled="ro.suguanzhiji" v-model="ruleForm.suguanzhiji" placeholder="请选择宿管职级" >
						<el-option
							v-for="(item,index) in suguanzhijiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="宿管职级" prop="suguanzhiji">
					<el-input v-model="ruleForm.suguanzhiji"
						placeholder="宿管职级" readonly></el-input>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='suguanzhiji'){
							this.ruleForm.suguanzhiji = obj[o];
							this.ro.suguanzhiji = true;
							continue;
						}
						if(o=='touxiang'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `suguan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.suguanCrossAddOrUpdateFlag = false;
						 this.parent.search();
