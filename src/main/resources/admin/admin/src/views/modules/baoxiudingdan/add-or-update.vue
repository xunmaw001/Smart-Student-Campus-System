<template>
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="报修编号" prop="baoxiubianhao">
					<el-input v-model="ruleForm.baoxiubianhao" placeholder="报修编号" clearable  :readonly="ro.baoxiubianhao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="报修编号" prop="baoxiubianhao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="处理状态" prop="chulizhuangtai">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="处理状态" prop="chulizhuangtai">
					<el-input v-model="ruleForm.chulizhuangtai"
						placeholder="处理状态" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="处理时间" prop="chulishijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.chulishijian" 
						type="datetime"
						:readonly="ro.chulishijian"
						placeholder="处理时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.chulishijian" label="处理时间" prop="chulishijian">
					<el-input v-model="ruleForm.chulishijian" placeholder="处理时间" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="处理简述" prop="chulijianshu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.chulijianshu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.chulijianshu" label="处理简述" prop="chulijianshu">
                    <span :style='{"fontSize":"14px","lineHeight":"1.5","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.chulijianshu"></span>
                </el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.chulishijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='chulijianshu'){
							this.ruleForm.chulijianshu = obj[o];
							this.ro.chulijianshu = true;
							continue;
						}
						if(o=='chulizhuangtai'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.chulijianshu = this.ruleForm.chulijianshu.replace(reg,'../../../springbootggr80/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baoxiudingdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baoxiudingdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
