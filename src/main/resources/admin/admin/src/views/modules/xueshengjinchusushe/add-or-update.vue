	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="类型" prop="leixing">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="类型" prop="leixing">
					<el-input v-model="ruleForm.leixing"
						placeholder="类型" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="口令码" prop="koulingma">
					<el-input v-model="ruleForm.koulingma" placeholder="口令码" clearable  :readonly="ro.koulingma"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="口令码" prop="koulingma">
					<el-input v-model="ruleForm.koulingma" placeholder="口令码" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="登记时间" prop="dengjishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.dengjishijian" label="登记时间" prop="dengjishijian">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="宿管账号" prop="suguanzhanghao">
					<el-select :disabled="ro.suguanzhanghao" @change="suguanzhanghaoChange" v-model="ruleForm.suguanzhanghao" placeholder="请选择宿管账号">
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.suguanzhanghao" label="宿管账号" prop="suguanzhanghao">
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				...
				if (data && data.code === 0) {
					this.suguanzhanghaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        url: `xueshengjinchusushe/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengjinchusushe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengjinchususheCrossAddOrUpdateFlag = false;
						 this.parent.search();
