	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="安排时间" prop="anpaishijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.anpaishijian" 
						type="datetime"
						:readonly="ro.anpaishijian"
						placeholder="安排时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.anpaishijian" label="安排时间" prop="anpaishijian">
					<el-input v-model="ruleForm.anpaishijian" placeholder="安排时间" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="维修账号" prop="weixiuzhanghao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.weixiuzhanghao" label="维修账号" prop="weixiuzhanghao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="问题简述" prop="wentijianshu">
					<editor 
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.wentijianshu" label="问题简述" prop="wentijianshu">
                    <span :style='{"fontSize":"14px","lineHeight":"1.5","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.wentijianshu"></span>
                </el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.anpaishijian = this.getCurDateTime()
	},
	methods: {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wentijianshu = this.ruleForm.wentijianshu.replace(reg,'../../../springbootggr80/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baoxiujiedan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baoxiujiedanCrossAddOrUpdateFlag = false;
						 this.parent.search();
