...
<div :style='{"border":"1px solid #dfdfdf","padding":"20px","margin":"20px auto 0","borderRadius":"16px","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="进来时间" prop="jinlaishijian" >
              <el-date-picker
                  value-format="yyyy-MM-dd HH:mm:ss"
                  v-model="ruleForm.jinlaishijian" 
                  type="datetime"
                  placeholder="进来时间">
              </el-date-picker>
          </el-form-item>
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="停留时间" prop="tingliushijian">
            <el-input v-model="ruleForm.tingliushijian" 
                placeholder="停留时间" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="教师账号" prop="jiaoshizhanghao">
            <el-select  @change="jiaoshizhanghaoChange" v-model="ruleForm.jiaoshizhanghao" placeholder="请选择教师账号">
              <el-option
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.jinlaishijian = this.getCurDateTime()
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jinlaishijian'){
              this.ruleForm.jinlaishijian = obj[o];
              this.ro.jinlaishijian = true;
              continue;
            }
            if(o=='tingliushijian'){
              this.ruleForm.tingliushijian = obj[o];
              this.ro.tingliushijian = true;
              continue;
            }
        ...
        this.$http.get('option/jiaoshi/jiaoshizhanghao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.jiaoshizhanghaoOptions = res.data.data;
          }
        });
      },
      // 下二随
      ...
      jiaoshizhanghaoChange () {
        this.$http.get('follow/jiaoshi/jiaoshizhanghao?columnValue=' + this.ruleForm.jiaoshizhanghao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.suguanxingming){
              this.ruleForm.suguanxingming = res.data.data.suguanxingming
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('daoyuanqueren/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('daoyuanqueren/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('daoyuanqueren/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
