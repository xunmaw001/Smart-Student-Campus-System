...
<div :style='{"border":"1px solid #dfdfdf","padding":"20px","margin":"20px auto 0","borderRadius":"16px","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="签到时间" prop="qiandaoshijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="宿管账号" prop="suguanzhanghao">
            <el-select  @change="suguanzhanghaoChange" v-model="ruleForm.suguanzhanghao" placeholder="请选择宿管账号">
              <el-option
                  v-for="(item,index) in suguanzhanghaoOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"border":"1px solid #dfdfdf","padding":"10px","boxShadow":"1px 2px 3px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"radial-gradient(circle, rgba(246,246,246,1) 0%, rgba(230,230,230,1) 100%)"}' label="地址" prop="fulladdress">
...
import { Loading } from 'element-ui';
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          suguanzhanghao: [
            { required: true, message: '宿管账号不能为空', trigger: 'blur' },
          ],
          suguanxingming: [
          ],
          longitude: [
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/suguan/suguanzhanghao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.suguanzhanghaoOptions = res.data.data;
          }
        });
      },
      // 下二随
      suguanzhanghaoChange () {
        this.$http.get('follow/suguan/suguanzhanghao?columnValue=' + this.ruleForm.suguanzhanghao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.suguanxingming){
              this.ruleForm.suguanxingming = res.data.data.suguanxingming
            }
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('xueshengqiandao/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('xueshengqiandao/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('xueshengqiandao/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
